...
# =========================
def logical_or(x: np.ndarray) -> int:
    """
    Логическая функция OR для вектора из 0 и 1.
    Возвращает 1, если хотя бы один элемент равен 1, иначе 0.
    """
...
def generate_truth_table(n: int):
    ...
    y: значения функции OR
    """
    combinations = list(itertools.product([0, 1], repeat=n))
    X = np.array(combinations, dtype=float)
    y = np.array([logical_or(row) for row in X], dtype=float).reshape(-1, 1)
    return X, y
...
class SingleLayerPerceptron:
    def __init__(self, n_inputs: int):
        """
        Инициализация весов и порога (bias).
        """
    ...
    def train_fixed_lr(self, X_train, y_train, X_test, y_test, learning_rate=0.1, max_epochs=1000):
        ...
        Возвращает историю ошибок на train и test.
        """
        train_errors = []
        test_errors = []
        ...
        for epoch in range(max_epochs):
            ...
            # Подсчет ошибок после обновления
    ...
    def train_adaptive_lr(self, X_train, y_train, X_test, y_test, initial_lr=0.5, max_epochs=1000):
        ...
        Если ошибка уменьшается, шаг немного увеличивается.
        Если ошибка растет, шаг уменьшается.
        """
        train_errors = []
        test_errors = []
        ...
        for epoch in range(max_epochs):
            ...
            if train_loss < prev_train_loss:
                ...
                # Повторное обновление с меньшим шагом
                self.weights += learning_rate * grad_w
                self.bias += learning_rate * grad_b
                new_train_pred = self.forward(X_train)
                train_loss = mse_loss(y_train, new_train_pred)
...
def print_dataset_examples(X, y, count=10):
    """
    Печатает несколько примеров из таблицы истинности.
    """
...
def main():
    ...
    print(f"Вариант 25: n = {N_INPUTS}, логическая функция OR")
    ...
    # 3. Эксперименты с фиксированным шагом
    fixed_results = []
    ...
    # 4. Эксперимент с адаптивным шагом
    np.random.seed(RANDOM_SEED)
    ...
    # 6. Выбираем лучшую модель по точности на test
    ...
    # 7. Проверка обобщающей способности на нескольких примерах из test
